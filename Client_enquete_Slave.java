...
public class Client_enquete_Slave extends Thread {
			...
			private int id;
			private int old_demande;

			public Client_enquete_Slave(Client_itf c, int id, Client_itf origin, int old_demande){
				this.c = c;
				this.id = id;
				this.origin = origin;
				this.old_demande = old_demande;
			}
			...
			public void run(){
				try {
					...
					origin.addReponse(obj, this.c, this.id, new_version, this.old_demande); /* Nous ajoutons la réponse de l'enquête dans le client qui l'a lancée */
				} catch (Exception e) {
				}
			}
